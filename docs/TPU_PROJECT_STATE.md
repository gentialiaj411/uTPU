...
- `firmware/host/compiler_introspection.py` - structured compiler-pipeline inspection for FX graph, Graph IR, compile plan, runtime plan, backend lowering, fallback ops, and uTPU ISA footprint.
...
- `firmware/host/compiler_introspection.py` - report generator that makes the supported compiler boundary visible without claiming broad model support or performance wins.
...
- `firmware/host/test_compiler_introspection.py` - tests that the inspection report exposes FX, Graph IR, lowered ops, zero fallback ops, CUDA backend metadata, and uTPU instruction footprint.
...
- `examples/inspect_compiler_pipeline.py` - one-command compiler pipeline inspection for the tiny integer MLP; writes `build/reports/compiler_introspection_tiny_mlp.json`.
...
- `tiny_mlp`: PyTorch eager `0.0454 ms`, torch matmul/cuBLAS-style `0.0731 ms`, compiled steady state `0.5689 ms`, compiled kernel `0.2058 ms`, H2D/D2H counts `1/1`, compiled-vs-quantized-reference error `0.0`, quantized-reference-vs-float-PyTorch error `2.0`, fallback ops `[]`; compiled steady state lost to both measured baselines. `torch.compile` was skipped because Triton was unavailable.
- `fc1_like_small`: PyTorch eager `0.1101 ms`, torch matmul/cuBLAS-style `0.0387 ms`, compiled steady state `0.7176 ms`, compiled kernel `0.3138 ms`, H2D/D2H counts `1/1`, compiled-vs-quantized-reference error `0.0`, quantized-reference-vs-float-PyTorch error `353.0`, fallback ops `[]`; compiled steady state lost to both measured baselines.
- `fc2_like_small`: PyTorch eager `0.0930 ms`, torch matmul/cuBLAS-style `0.0638 ms`, compiled steady state `0.6224 ms`, compiled kernel `0.2135 ms`, H2D/D2H counts `1/1`, compiled-vs-quantized-reference error `0.0`, quantized-reference-vs-float-PyTorch error `322.0`, fallback ops `[]`; compiled steady state lost to both measured baselines.
- `stress_256_256_128`: PyTorch eager `0.0549 ms`, torch matmul/cuBLAS-style `0.0441 ms`, compiled steady state `0.7979 ms`, compiled kernel `0.3151 ms`, H2D/D2H counts `1/1`, compiled-vs-quantized-reference error `0.0`, quantized-reference-vs-float-PyTorch error `1254.0`, fallback ops `[]`; compiled steady state lost to both measured baselines.
...
- The opt-in tuned compiled MLP benchmark did not improve steady-state wall time in this run: `tiny_mlp` fixed kernel `0.5782 ms` vs tuned `1.1474 ms`, fixed steady `0.9217 ms` vs tuned `2.5851 ms`; `fc1_like_small` fixed kernel `0.2165 ms` vs tuned `0.4566 ms`, fixed steady `0.4753 ms` vs tuned `1.8241 ms`; `fc2_like_small` fixed kernel `0.2324 ms` vs tuned `0.1877 ms`, fixed steady `0.5284 ms` vs tuned `1.5368 ms`. Fixed schedule remains available and is still the default.
...
- `python firmware/host/test_compiler_introspection.py`
...
- 2026-05-15: Added compiler pipeline introspection. `examples/inspect_compiler_pipeline.py` now prints and writes a report covering FX nodes, Graph IR ops, blocked schedules, fallback/unsupported ops, CUDA backend metadata, and uTPU instruction footprint. README framing was tightened around honest scope, why CUDA exists, and what is not claimed.
