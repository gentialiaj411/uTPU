...
- Bounded-jitter latency (iverilog RTL sim, not on-board): cycle-invariant end-to-end inference across adversarial + random inputs at the measured 100 MHz close; vs-GPU tail comparison records median-latency loss and does not claim FPGA speedup. Evidence: [bench/results/latency_determinism.json](bench/results/latency_determinism.json), [bench/results/latency_determinism_vs_gpu.json](bench/results/latency_determinism_vs_gpu.json), [docs/latency_determinism_vs_gpu_logx.png](docs/latency_determinism_vs_gpu_logx.png).
